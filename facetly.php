...
class Facetly extends Module{
	...
	public function getContent(){
		...
		if (Tools::isSubmit('submitReindex')){		
			...
			echo '<meta http-equiv="refresh" content="3;URL='.$_SERVER['PHP_SELF'].'?tab='.$tab.'&configure='.$configure.'&token='.$token.'&tab_module='.$tab_module.'&module_name='.$module_name.'&page=0">';
		}
			...
			if($page < $max_page) {				
				$page = $page + 1;
				echo '<meta http-equiv="refresh" content="3;URL='.$_SERVER['PHP_SELF'].'?tab='.$tab.'&configure='.$configure.'&token='.$token.'&tab_module='.$tab_module.'&module_name='.$module_name.'&page='.$page.'">';
			}else{
				echo '<meta http-equiv="refresh" content="3;URL='.$_SERVER['PHP_SELF'].'?tab='.$tab.'&configure='.$configure.'&token='.$token.'&tab_module='.$tab_module.'&module_name='.$module_name.'">';
			}
		}
		else{
			$this->_displayForm();
	...
	public function hookLeftColumn(){		
		...
		$action = __PS_BASE_URI__ . "modules/facetly/find.php";
		// fix issue on jquery address		
		if (__PS_BASE_URI__ . "modules/facetly/find.php" == $_SERVER['SCRIPT_NAME']) {
			$action = "find.php";
		}
		...
		$facetly_block = '			
				<div class="block"><h4>Search</h4>
					<div class="block_content">
						<form action="'.$action.'" facetly_form="on" method="get">
